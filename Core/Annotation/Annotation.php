...
class Annotation {
    ...
    public static function getClass($class) {
        ...
        foreach ($dataClasses as $key => $dataClass) {
            ...
            array_push($data['class'], [0 => ['annotation' => $dataClass['type'], 'instance' => $processorClass->process()]]);
        }
        ...
        foreach ($methodsName as $methodName) {
            ...
            if (count($dataMethods) > 0) {
                ...
                foreach ($dataMethods as $key => $dataMethod) {
                    ...
                    array_push($data['methods'][$methodName->getName()], ['annotation' => $dataMethod['type'], 'instance' => $processorMethods->process()]);
                }
            }
        }
        ...
        foreach ($propertiesNames as $propertyName) {
            ...
            if (count($dataProperties) > 0) {
                ...
                foreach ($dataProperties as $key => $dataProperty) {
                    ...
                    array_push($data['properties'][$propertyName->getName()], ['annotation' => $dataProperty['type'], 'instance' => $processorProperties->process()]);
                }
            }
        }
