...
namespace Gcs\Framework\Core\Sql;
...
class Sql {
    ...
    public function fetch($name, $fetch = self::PARAM_FETCH) {
        ...
        if ((isset($this->_cache) && $this->_cache->isDie() && $this->_time[$name] > 0) || $this->_time[$name] == 0 || $fetch == self::PARAM_FETCHINSERT || $fetch == self::PARAM_FETCHUPDATE || $fetch == self::PARAM_FETCHDELETE) {
            ...
            try {
                ...
                switch ($fetch) {
                    ...
                    case self::PARAM_FETCHCOLUMN :
                        $this->_data = $query->fetchColumn();
                        break;
                    case self::PARAM_FETCHINSERT :
                        $this->_data = true;
                        break;
                    case self::PARAM_FETCHUPDATE :
                        $this->_data = true;
                        break;
                    case self::PARAM_FETCHDELETE :
                        $this->_data = true;
                        break;
                    case self::PARAM_NORETURN :
                        $this->_data = true;
                        break;
                    default :
                        $this->addError('the execution constant ' . $fetch . ' doesn\'t exist', __LINE__, __FILE__, ERROR_INFORMATION, LOG_SQL);
                        break;
                }
