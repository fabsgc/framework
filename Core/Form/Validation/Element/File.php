...
class File extends Element {
    ...
    public function __construct($field, $label) {
        ...
        if (!isset($this->_data[$field])) {
            array_push($this->_errors, ['field' => $this->_label, 'message' => Lang::instance()->lang('.app.system.form.exist')]);
                        ...
                        foreach ($this->_data[$this->_field]['type'] as $value) {
                            if ($value != '') {
                                if (!in_array($value, $constraint['value'])) {
                                    array_push($this->_errors, ['name' => $this->_field, 'field' => $this->_label, 'message' => $constraint['message']]);
                                }
                            }
                        }
                        break;
    ...
    public function sizeMax($sizeMax, $error) {
        if ($this->_exist) {
            array_push($this->_constraints, ['type' => self::SIZEMIN, 'value' => $sizeMax, 'message' => $error]);
        }
