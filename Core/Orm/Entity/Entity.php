...
use Gcs\Framework\Core\Exception\MissingFieldException;
...
abstract class Entity {
    ...
    public function hydrate($prefix = '') {
        ...
        if (isset($this->_data[$prefix . $this->primary()])) {
            ...
            $data = $entityName::find()->where($field . ' = :id')->vars(['id' => $this->_data[$table . '_' . $this->primary()]])->fetch()->first();
                ...
                switch ($field->foreign->type()) {
                    case ForeignKey::ONE_TO_ONE : {
                        ...
                        if (isset($this->_data[$fieldName])) {
                            if ($this->_data[$fieldName] != '') {
                                ...
                                $field->value = $builder->find()->where($fieldFormName . ' = :id')->vars(['id' => $this->_data[$fieldName]])->fetch()->first();
                    ...
                    case ForeignKey::ONE_TO_MANY :
                        if (isset($this->_data[$fieldName])) {
                            ...
                            $builder = new Builder($entityJoin);
                            $field->value = $builder->find()->where($fieldFormName . ' IN(' . $in . ')')->vars($inVars)->fetch();
                        }
                        break;
